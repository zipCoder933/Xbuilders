...
public class DefaultTerrainUtils {
    ...
    public static void plantBirchOrOakTree(GenSession session, final Chunk sourceChunk, final int x, final int y, final int z) {
        if (session.random.nextBoolean()) {
            BirchTreeUtils.plantTree(session, sourceChunk, x, y, z);
        } else {
            OakTreeUtils.plantTree(session, sourceChunk, x, y, z);
        }
    }
