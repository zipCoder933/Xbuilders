...
public class BlockModel {
    ...
    public void render(BufferSet buff, Block block, Block[] neighbors, int x, int y, int z) {
        ...
        for (int side = 0; side < sides.length; side++) {
            if (sides[side] != null &&
                ...
                switch (sides[side].textureSide) {
                    case BlockType.POS_X -> texture = block.texture.getPOS_X();
                    case BlockType.NEG_X -> texture = block.texture.getNEG_X();
                    case BlockType.POS_Z -> texture = block.texture.getPOS_Z();
                    case BlockType.NEG_Z -> texture = block.texture.getNEG_Z();
                    case BlockType.POS_Y -> texture = block.texture.getPOS_Y();
                    case BlockType.NEG_Y -> texture = block.texture.getNEG_Y();
                    default -> texture = block.texture.getPOS_Y();
                }
                sides[side].render(x, y, z, buff, texture);
            }
        }
    }
}
